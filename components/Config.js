...
const Plugin_Name = 'deanqwq233-plugin'
const Plugin_Path = `${Path}/plugins/${Plugin_Name}`;

if(!Bot.deanqwq233_plugin) Bot.deanqwq233_plugin = {};

class Config {
	constructor () {
    this.config = {}
	
	Bot.deanqwq233_plugin['config'] = this.config;
  ...
  getYaml (app, name, type) {
    ...
    try {
      ...
      logger.error(`[水稻插件][${app}][${name}] 格式错误 ${error}`)
      return false
    }
  ...
  watch (file, app, name, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[水稻插件][修改配置文件][${type}][${app}][${name}]`)
