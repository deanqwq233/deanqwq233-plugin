...
export default async function (path, params, cfg) {
  ...
  let data = {
    ...
    sys: {
      scale: 1,
      copyright: `Created By Yunzai-Bot<span class="version">${Version.yunzai}</span> & deanqwq233-Plugin<span class="version">${Version.ver}</span>`
    },
    quality: 100
  }
  ...
  let img = await deanqwq233_plugin.puppeteer.screenshot(`${Plugin_Name}/${app}/${tpl}`, data)
  let ret = true
  if (img) {
